...
import numpy as np
from jiangxi_competition.map.first.operation import Lfun
...
class PyLMDI():
    ...
    def Add(self):
        ...
        for start, end in zip(self.X0, self.Xt):
            temp = sum([Lfun(self.Vt[i], self.V0[i]) * np.log(end[i] / start[i]) for i in range(len(start))])
            Delta_V.append(temp)
        return Delta_V
    ...
    def Mul(self):
        ...
        for start, end in zip(self.X0, self.Xt):
            temp = sum(
                [Lfun(self.Vt[i], self.V0[i]) / Lfun(sum(self.Vt), sum(self.V0)) * np.log(end[i] / start[i]) for i in
                 range(len(start))])
